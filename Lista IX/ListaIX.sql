...
FROM
  ...
  autor au
WHERE
  es.codigo = cp.codEstoque
  AND es.codEditora = ed.codigo
...
SELECT
  es.nome AS livro,
  site = CASE
    WHEN (LEN (ed.site) > 10) THEN SUBSTRING(ed.site, 5, LEN (ed.site))
    ELSE ed.site
  END
FROM
  estoque es,
...
FROM
  ...
  estoque es
WHERE
  cp.codEstoque = es.codigo
  AND es.nome = 'Sistemas Operacionais Modernos'
--6) Consultar quais livros não foram vendidos	
SELECT
  es.nome AS livronaovendido
FROM
  estoque es
  LEFT OUTER JOIN compra cp ON es.codigo = cp.codEstoque
WHERE
  cp.codEstoque IS NULL
...
SELECT
  RTRIM(es.nome) AS livro,
  cp.codigo,
  cp.codEstoque
FROM
  compra cp
  LEFT OUTER JOIN estoque es ON cp.codEstoque = es.codigo
WHERE
  es.codigo IS NULL
--8) Consultar Nome e site da editora que n�o tem Livros no estoque (Caso o site tenha mais de 10 dígitos, remover o www.)	
SELECT
  ed.nome,
  site = CASE
    WHEN (LEN (ed.site) > 10) THEN SUBSTRING(ed.site, 5, LEN (ed.site))
...
FROM
  editora ed
  LEFT OUTER JOIN estoque es ON es.codEditora = ed.codigo
WHERE
...
SELECT
  au.nome,
  CASE
    WHEN (au.biografia LIKE 'Doutorado%') THEN 'Ph.D' + SUBSTRING(au.biografia, 10, LEN (au.biografia))
    ELSE au.biografia
  END AS biografia
FROM
  autor au
  LEFT OUTER JOIN estoque es ON es.codAutor = au.codigo
WHERE
  es.codAutor IS NULL
...
SELECT
  cp.codigo,
  SUM(cp.qtdComprada) AS totallivros,
  SUM(cp.valor) AS somavalores
FROM
  compra cp
GROUP BY
  cp.codigo
...
SELECT
  ed.nome,
  CAST(AVG(es.valor) AS DECIMAL(4, 1)) AS mediapreco
FROM
  editora ed,
  estoque es
WHERE
  ed.codigo = es.codEditora
GROUP BY
  ed.nome
ORDER BY
  mediapreco ASC
...
SELECT
  ...
  CASE
    ...
    AND es.quantidade <= 10 THEN 'Produto Acabando'
...
SELECT
  ...
  au.nome AS autor,
  CASE
    WHEN (ed.site IS NOT NULL) THEN ed.nome + ' ' + ed.site
    ELSE 'Não possui site'
  END AS infoeditora
FROM
  estoque es,
  autor au,
...
WHERE
  es.codEditora = ed.codigo
  AND es.codAutor = au.codigo
--15) Consultar Código da compra, quantos dias da compra até hoje e quantos meses da compra até hoje	
SELECT
  cp.codigo,
  DATEDIFF(DAY, cp.dataCompra, GETDATE ()) AS diasdacompra,
