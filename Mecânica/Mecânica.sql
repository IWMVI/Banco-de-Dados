...
CREATE TABLE veiculo 
    (
        ...
        (
            ...
            AND (ano_modelo <= (ano_fabricacao + 1))
        ),
        PRIMARY KEY (placa),
...
CREATE TABLE categoria 
    (
        ...
        PRIMARY KEY (id),
        CONSTRAINT check_categoria_valorhora CHECK 
        (
            (
                UPPER(categoria) = 'ESTAGIÁRIO'
                AND valor_hora > 15
            )
            OR (
                UPPER(categoria) = 'NÍVEL 1'
                AND valor_hora > 25
            )
            OR (
                UPPER(categoria) = 'NÍVEL 2'
                AND valor_hora > 35
            )
            OR (
...
CREATE TABLE funcionario 
    (
        ...
        telefone CHAR(11) NOT NULL CHECK 
        (
            LEN (telefone) = 10
            OR LEN (telefone) = 11
        ),
        catego_habilitacao VARCHAR(2) NOT NULL CHECK 
        (
            (UPPER(catego_habilitacao) = 'A')
            OR (UPPER(catego_habilitacao) = 'B')
            OR (UPPER(catego_habilitacao) = 'C')
            OR (UPPER(catego_habilitacao) = 'D')
            OR (UPPER(catego_habilitacao) = 'E')
        ),
        id_categoria INT NOT NULL PRIMARY KEY (id) FOREIGN KEY (id_categoria) REFERENCES categoria (id)
    )
