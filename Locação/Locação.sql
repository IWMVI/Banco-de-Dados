...
(
  ...
  CONSTRAINT CHK_locacao_datadevolucao CHECK ((data_devolucao > data_locacao)),
...
VALUES
  (
    'Alexandre e o Dia Terr�vel, Horr�vel, Espantoso e Horroroso',
    2014
  )
INSERT INTO
  filme
...
UPDATE estrela
SET
  nomereal = 'Miles Alexander Teller'
WHERE
  nome = 'Miles Teller'
DELETE FROM filme
WHERE
...
SELECT
  filmeID,
  CONVERT(CHAR(12), data_fabricacao, 103) AS fab
FROM
  dvd
WHERE
...
SELECT
  num_cadastro,
  nome,
  logradouro + ' ' + CONVERT(VARCHAR(5), num) + ' | ' + cep AS end_comp
FROM
  cliente
WHERE
  num_cadastro >= 5503